			...
			$.getJSON(
				...
				function(data) {
					...
					if(data && typeof data === 'object' && data.hasOwnProperty('query') && data.query.hasOwnProperty('results') && data.query.results.hasOwnProperty('channel')) {
						...
						$this.find("span.revslider_data_weather_icon").html('<i className=\"revslider-weather-icon revslider-weather-icon-' + weather_info["item"]["forecast"][0]["code"] + '\"></i>');
						...
						for(var i=0;i<10;i++){
							...
							$this.find("span.revslider_data_weather_icon_forecast_"+i).html('<i className=\"revslider-weather-icon revslider-weather-icon-' + weather_info["item"]["forecast"][i]["code"] + '\"></i>');
